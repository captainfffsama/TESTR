...
from adet.layers.deformable_transformer import DeformableTransformer,DeformableTransformerNumPoint
...
class TESTR_NUMPOINT(nn.Module):
    """
    Same as :class:`detectron2.modeling.ProposalNetwork`.
    Use one stage detector and a second stage for instance-wise prediction.
    """
    ...
    def __init__(self, cfg, backbone):
        ...
        self.transformer = DeformableTransformerNumPoint(
            d_model=self.d_model,
            nhead=self.nhead,
            num_encoder_layers=self.num_encoder_layers,
            num_decoder_layers=self.num_decoder_layers,
    ...
    def forward(self, samples: NestedTensor):
        ...
        out["enc_outputs"] = {
            "pred_logits": enc_outputs_class,
            "pred_proposals": enc_outputs_coord,
        }
        return out
