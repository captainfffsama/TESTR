...
from adet.modeling.testr.losses import SetCriterion,NumPointCriterion
from adet.modeling.testr.matcher import (
    build_matcher,
    BoxHungarianMatcher,
    CtrlPointHungarianMatcher,
    ProposalHungarianMatcher,
)
from adet.modeling.testr.models import TESTR, TESTR_NUMPOINT
...
class TransformerDetector(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        """
        # batched_inputs is a list,length is batch size,element is a dict
        # element dict has key:
        #   file_name:str,
        #   height:int,
        #   width:int,
        #   image_id:int,
        ...
        # instances like this:
        # Instances(num_instances=1, image_height=2333, image_width=879,
        #   fields=[gt_boxes: Boxes(tensor([[ 98., 102., 303., 165.]])),
        #       gt_classes: tensor([0]),
        #       text: tensor([[35, 65, 82, 80, 65, 82, 75, 96, 96, 96, 96,
        #               96, 96, 96, 96, 96, 96, 96,96, 96, 96, 96, 96, 96, 96]], dtype=torch.int32),
        #       polygons: tensor([[ 98.1624, 102.5985, 149.4746, 102.5985, 200.7868, 102.5985, 252.0990,
        #               102.5985, 303.4112, 102.5985, 303.4112, 118.2113, 303.4112, 133.8241,
        #               303.4112, 149.4369, 303.4112, 165.0497, 252.0990, 165.0497, 200.7868,
        #               165.0497, 149.4746, 165.0497,  98.1624, 165.0497,  98.1624, 149.4369,
        #               98.1624, 133.8241,  98.1624, 118.2113]])])
...
@META_ARCH_REGISTRY.register()
class TransformerDetectorNumPointNet(nn.Module):
    """
    Same as :class:`detectron2.modeling.ProposalNetwork`.
    Use one stage detector and a second stage for instance-wise prediction.
    """
    ...
    def __init__(self, cfg):
        ...
        backbone = Joiner(d2_backbone, PositionalEncoding2D(N_steps, normalize=True))
        backbone.num_channels = d2_backbone.num_channels
        self.testr = TESTR_NUMPOINT(cfg, backbone)


        proposals_matcher = ProposalHungarianMatcher(
            class_weight=cfg.MODEL.TRANSFORMER.LOSS.POINT_CLASS_WEIGHT,
            coord_weight=cfg.MODEL.TRANSFORMER.LOSS.POINT_COORD_WEIGHT,
            focal_alpha=cfg.MODEL.TRANSFORMER.LOSS.FOCAL_ALPHA,
            focal_gamma=cfg.MODEL.TRANSFORMER.LOSS.FOCAL_GAMMA,
        )
        ...
        enc_losses = ["labels", "proposals"]
        dec_losses = ["labels", "ctrl_points", "texts"]

        self.criterion = NumPointCriterion(
            self.testr.num_classes,
            proposals_matcher ,
            point_matcher,
            weight_dict,
