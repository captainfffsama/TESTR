...
class DeformableTransformer(nn.Module):
    ...
    def gen_encoder_output_proposals(
        ...
        for lvl, (H_, W_) in enumerate(spatial_shapes):
            ...
            # 这里0.05是经验性的值,用来确定proposal的初始大小
            wh = torch.ones_like(grid) * 0.05 * (2.0**lvl)
            proposal = torch.cat((grid, wh), -1).view(N_, -1, 4)
            proposals.append(proposal)
            _cur += H_ * W_

        # b,all shape level hw,4
...
class DeformableCompositeTransformerDecoderLayer(nn.Module):
    ...
    def forward_ffn_text(self, tgt):
        tgt2 = self.linear2_text(
            self.dropout3_text(self.activation_text(self.linear1_text(tgt)))
        )
    ...
    def forward(
        ...
        # reference_points:   batch_size, n_objects, size level, 4
        ...
        # cross attention
        # b,obj_num,16,size level,4
        reference_points_loc = reference_points[:, :, None, :, :].repeat(
            1, 1, tgt_inter.shape[2], 1, 1
        )
        # after attn_cross is b,1600,256
        ...
        reference_points_text = reference_points[:, :, None, :, :].repeat(
            1, 1, tgt_text_inter.shape[2], 1, 1
        )
        tgt2_text_cm = self.attn_cross_text(
            self.with_pos_embed(tgt_text_inter, query_pos_text).flatten(1, 2),
            reference_points_text.flatten(1, 2),
            src,
            src_spatial_shapes,
            level_start_index,
            src_padding_mask,
        ).reshape(tgt_text_inter.shape)
...
class DeformableCompositeTransformerDecoder(nn.Module):
    ...
    def forward(
        ...
        query_pos_text: Optional[torch.Tensor] = None,
        ...
        Args:
            tgt (torch.Tensor): b,topk,16,c(256) control point query embedding.
            tgt_text (torch.Tensor): b,topk,max_text_len,c(256) text query embedding,the weight of nn.embedding,not through encoder.
            reference_points (torch.Tensor): b,topk,4. proposal box,xywh
            src (torch.Tensor): b,hw,c(256) image feature after encoder
            src_spatial_shapes (torch.Tensor): size level(4),2. record each size level shape.from big to small.
            src_level_start_index (torch.Tensor): size level(4),mark start index in src 1 dim.
            src_valid_ratios (torch.Tensor): b,size level(4),2. record each image each level valid w and h.
            query_pos (Optional[torch.Tensor], optional): b,obj_num(100),16,c(256). control point query,2 bezier line,each have 8 points.this come from reference_points add query pos embedding.Defaults to None.
            query_pos_text (Optional[torch.Tensor], optional): obj_num(100),max_text_len,c(256). text pos embedding.Defaults to None.
            src_padding_mask (Optional[torch.Tensor], optional): b,hw. mark the valid region in src. Defaults to None.
            text_padding_mask (_type_, optional): _description_. Defaults to None.
